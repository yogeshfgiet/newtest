  ...
  "orderable": false
            ...
            if (0 < ($('#posted_showings_list')).length) {
              ...
              "render": function (data, type, row, meta) {
                ...
                /* Showings lebels for posting agent */
                
                var rejected =row.rejected_id;
                var  showingUserId =row.showing_user_id;
                if(rejected && showingUserId == 0 || row.rejected_showing_agent_id == showingUserId || row.showing_progress == 0 || row.showing_progress == 3){

                  var editIcon =  '<button style=" width:90%; border-color: #E7E6E6;cursor:not-allowed; "  type="button" title="Offer Declined" class="btn btn-xs btn-info"> Open Posting </button>';
                } // Changed to offer declined to Open Posting
                else{
                  if(row.showing_progress == 1) {
                    var editIcon =  '<button style="width:90%; background-color:#acf1be " type="button" onclick="acceptedUserData(' + row.showing_user_id + ','+data+')"' + 'title="Offer Recieved" class="btn btn-xs btn-default">Offer Recieved</button> ';
                  }
                  else if(row.showing_progress == 2){
                    var editIcon =  '<button style="width:90%;" type="button" onclick="viewAcceptedUserData(' + row.showing_user_id + ','+data+')"' + 'title="Offer Approved" class="btn btn-xs btn-success">Offer Approved <br>Review Agent </button>  ';
                  }
                    ...
                    var editIcon = '<button  style="width:90%;background-color:#984a4a; color:white ; cursor:not-allowed ; padding-left:5px" type="button" title="Payment Blocked" class="btn btn-xs btn-default">Payment Blocked</button>';
                  }
                  else if(row.showing_progress == 6){
                    var editIcon =  '<button  style="width:90%; " type="button" onclick="viewCompleteShowings('+ data +')"' + '  title="Posting Completed" class="btn btn-xs btn-success">Posting Completed</button>';
                  }
                  else{
                    var  editIcon = "";
                  }
        ...
        if(rejected && showingUserId == 0 || currentUserId == row.rejected_showing_agent_id ){
          ...
          if(row.showing_progress == 1){
           ...
           if(row.post_date > currentDate  ){
             var editIcon = '<button style="width:90%; background-color:#E7E6E6; border-color: #E7E6E6; color:geen ; background-color:#5cb85c" type="button" onclick="getFeedbackForm(' + data + ')"' + 'title="Offer Accepted" class="btn btn-xs btn-default"> Offer Accepted <br> Complete Feedback </button> ';
           }
           else {
...
/*new block for acceted showings */
